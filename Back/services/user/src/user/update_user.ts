...
export async function update_user(user:User){
	...
	SET last_name=COALESCE($2,last_name), first_name=COALESCE($3,first_name), password=COALESCE($4,password), address=COALESCE($5,address), email=COALESCE($6,email), phone_number=COALESCE($7,phone_number), jwt=null
	WHERE id=$1;`;
