...
export default class OrderController {
    public getOrder(requestId:string, jwt:string): Promise<IResponse> | never {
        let fetch = new fetcher('https://mag_order:8094/order', 'GET', requestId);
        fetch.jwt = jwt;
        return fetch.call();
    }
    ...
    public createOrder(requestId:string, jwt:string,body:string): Promise<IResponse> | never {
        let fetch = new fetcher('https://mag_order:8094/order/create', 'PUT', requestId);
        fetch.jwt = jwt;
        fetch.body = body;
        return fetch.call();
    }
    public updateOrder(requestId:string, jwt:string,id:string,body:string): Promise<IResponse> | never {
        let fetch = new fetcher(`https://mag_order:8094/order/update/${id}`, 'PUT', requestId);
        fetch.jwt = jwt;
        fetch.body = body;
        return fetch.call();
    }
